...
## getter/setter functions that maintain the state of the current  input matrix
...
cacheSolve <- function(CMO, ...) 
{
    ...
    if(!is.null(ci)) 
    {
        print('for the current matrix of:')
        print(CMO$get())
        print('the cached inverse matrix is:')
        return(ci)
    }
    ...
    print('for the current matrix of: ')
    print(CMO$get())
    print('now solving for the inverse matrix... it is:')  
    return(inverse)
}
